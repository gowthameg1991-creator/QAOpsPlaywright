...
test("create event via UI, book it, and verify seat reduction", async ({ page }) => {
   ...
   const seatsBeforeBookingNumber = parseInt(await targetCard.getByText("seat").first().innerText());
   const seatsA = await targetCard.getByText("seat").last().innerText();
   console.log(`Seats before booking: ${seatsBeforeBookingNumber} (${seatsA})`);
   ...
   expect(seatsAfterBooking).toBe(seatsBeforeBookingNumber - 1);
});
